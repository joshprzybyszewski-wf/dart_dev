...
Future<GenTestRunnerTask> genTestRunner(TestRunnerConfig currentConfig) async {
  ...
  List<File> generatedRunners = [];
  if (currentConfig.numRunners > 1) {
    for (var i = 0; i < currentConfig.numRunners; i++) {
      generatedRunners.add(
          new File('${currentDirectory}${currentConfig.filename}_${i+1}.dart'));
    }
  } else {
    generatedRunners
        .add(new File('${currentDirectory}${currentConfig.filename}.dart'));
  }

  Map<File, List<String>> existingLines;
  if (currentConfig.check) {
    try {
      for (var generatedRunner in generatedRunners) {
        existingLines[generatedRunner] =
            generatedRunner.readAsStringSync().split('\n');
      }
    } catch (_) {
      existingLines = {};
    }
  ...
  var numTestFiles = testFiles.length;
  int filesPerRunner = (numTestFiles / currentConfig.numRunners).floor();
  var remainder = numTestFiles - (filesPerRunner * currentConfig.numRunners);

  print(
      '$numTestFiles total, $filesPerRunner per runner + remainder of $remainder');
  for (var i = 0; i < generatedRunners.length; i++) {
    var generatedRunner = generatedRunners[i];
    if (currentConfig.genHtml && !currentConfig.check) {
      print('generate HTML file for ${currentConfig.filename}_${i+1}.dart');
      await testHtmlFileGenerator(currentDirectory,
          "${currentConfig.filename}_${i+1}", currentConfig.htmlHeaders);
    } else {
      print(
          'do not generate HTML because ${currentConfig.genHtml} flag vs ${currentConfig.check} checkmode');
    }
    ...
    runnerLines
        .add('// Generated by `pub run dart_dev ${task.generateCommand}`');
    runnerLines.add('');

    int start = i * filesPerRunner;
    if (i > 0) start += remainder;
    for (var testFileIndex = start;
        testFileIndex < start + filesPerRunner;
        testFileIndex++) {
      var file = testFiles[testFileIndex];
    ...
    task.runnerFiles.add(generatedRunner.path);
    ...
    if (currentConfig.check) {
      ...
      for (int i = 0; i < existingLines.length; i++) {
        if (existingLines[generatedRunner][i] != runnerLines[i]) {
          success = false;
          task.mismatchedLineNumber = i + 1;
          task.newMismatchedLine = runnerLines[i];
          task.oldMismatchedLine = existingLines[generatedRunner][i];
          break;
        }
      }
